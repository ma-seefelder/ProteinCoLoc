...
- **D-15:** **(Spike-validated)** The first pipeline stage ("correlated bivariate densities", D-06) uses a **shared-latent correlated smooth Gaussian-field** generator: a smooth latent field `L` (Gaussian-blurred white noise) is mixed per channel — `c1 = softplus(√|ρ|·L + √(1−|ρ|)·ε₁)`, `c2 = softplus(sgn(ρ)·√|ρ|·L + √(1−|ρ|)·ε₂)` (softplus = `log1p∘exp` nonneg transform), with channel-2's shared component sign-flipped for negative ρ. A throwaway generator spike (run 2026-06-26, through the REAL `patch()`/`correlation()` at 8×8 after a representative PSF+background+noise degrade) showed this is **monotone (Spearman 1.0)** with induced-μ spanning **[−0.76, +0.76]** post-degrade (reaching both tails incl. anti-correlation), near-linear knob→μ ⇒ a well-behaved calibration inverse ĝ. The **puncta/spot-mixture generator is REJECTED** as the core generator: under the same realistic pipeline its induced-μ saturates at **~0.21** (sparse signal crushed by PSF+noise+`_exclude_zero`) — it cannot reach the Turing μ-prior and cannot produce negative correlation. Puncta is reserved only as a possible future realism variant, not the workhorse.
- **D-16:** **(Spike caveat → real anchor)** The real-image μ anchor must extract the **true per-channel signal the way `src/LoadImages.jl` does**, NOT an RGB→luminance reduction (the `_cN.tif` files load as RGB). Under the crude luminance proxy the spike saw real positive ≈ 0.33 and negative ≈ 0.25 — a narrow moderate band — so Phase 2 must (a) read channels faithfully before trusting the anchor, and (b) check whether the **negative tail of the μ-prior is physically reachable** at all in real fluorescence; this bears on how literally the D-01 consistency claim holds at the negative extreme (it may be acceptable to demonstrate consistency over the physically-realized μ range and document the negative tail as prior-only).
...
- **Generator de-risk spike (2026-06-26):** an isolated throwaway experiment compared the shared-latent smooth-field vs puncta-mixture generators through the REAL `patch()`/`correlation()`. Verdict locked in D-15 (shared-latent wins: monotone, induced-μ ∈ [−0.76,+0.76] post-degrade; puncta saturates ~0.21 and is rejected). This retired the #1 calibration-coverage risk BEFORE building the simulator.
