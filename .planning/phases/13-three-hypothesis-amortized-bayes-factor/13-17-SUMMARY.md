...
tags: [d-15-amended, d-16, pre-registration-amendment, real-images, include-guard, def-12-03, consts-sha, reported-not-gated, escalated]
requires:
...
decisions:
  ...
  - "Three test_p13_real.jl assertions broken by the corrected pair are LEFT FAILING rather than rewritten -- the disposition is escalated, not resolved in execution"
metrics:
  duration: "~2 h"
...
retracted two claims the phase had made**.

---

## ⚠ STATUS: `partial`, and why

**All six tasks executed and every deliverable exists.** The status is not `complete` because the
re-run surfaced a **pre-registration question that this plan deliberately did not resolve**, and
because three test assertions are now deliberately failing as a result. See
**§8 — the open blocker**. The plan index counts a plan complete on a file's mere existence, and
this project has been damaged by that before; `partial` is the honest label until the user rules.
...
**The suite aborts at `runtests.jl:223`, on `test_p13_real.jl`, and this plan caused that** — three
deliberate measured misses (§8). That masks the **seven** Phase-13 files after it, so **all seven
were run individually** and are reported here rather than assumed:
...
## 8. ⚠ THE OPEN BLOCKER — a pre-registration ruling is needed

**The corrected pair retracts two claims this phase made.** The three `test_p13_real.jl` assertions
that encode them were **left FAILING rather than rewritten**, because rewriting a substrate
expectation to match what was measured *after* the measurement is exactly the act the amendment
exists not to be.
...
**DECISION NEEDED — and this plan did not take it:** accept the three misses as named limits, amend
the D-16 real-substrate expectation, or re-order the harness so they stop masking siblings.
**No seed, bar, floor, band or allowance may be moved to resolve this**, and
`P13_ITERATION_ALLOWANCE` cannot be spent on a real-image observation (`13-SC2-AMENDMENT.md` §7).
Recorded additively in `STATE.md`.
