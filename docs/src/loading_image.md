...
A second, more convenient option is provived by the function `get_images(path::S, nchannels::I, stack_name::S; mask::Bool = true)`. This function retrieves and loads multi-channel images from the directory specified as `path`.

To correctly load images into using the function `get_images()`all images belonging to the same biological condition must be in the same folder and the individual channels should be named as $\left[ {\text{image name}} \right]\_\left[{\text{channel ID}} \right].{\text{tiff}}$. If there are additional underscores "_" in the file names, the characters following the last underscore are used as channel identifier. For example, 20231212_HAP40_channel_1.tiff, 20231212_HAP40_channel_2.tiff, and 20231212_HAP40_channel_3.tiff might be acceptable file names for images of the color channels 1, 2, and 3 for the sample 20231212_HAP40.
