...
"""
function _measured_maps(; G::Int = _P12RES_G, s::Float64 = 0.6, c::Float64 = 0.2,
                        sd::Float64 = 0.1)
...
_meta(; arm = :car, r1_posterior_median = 0.5, derived_rho = 0.3, ood_threshold = nothing,
...
_res(maps = _measured_maps(); draws = nothing, calibration = nothing, meta = _meta(),
     G::Int = _P12RES_G) =
...
@testset "P12 spatial result type and its three maps (SPAT-05)" verbose = true begin
    ...
    @testset "SpatialColocResultSpike is a real AbstractColocResult subtype (Phase-7 D-02)" begin
        ...
        @test RES_FIX isa AbstractColocResult
        ...
        @test_throws ErrorException bayes_factor(RES_FIX)
        let err = try; bayes_factor(RES_FIX); catch e; e; end
    ...
    @testset "an unscorable region is unmistakable (T-7-04), in ALL THREE maps" begin
        ...
        # measurement.
        @test_throws UndefKeywordError p12_mark_unscorable!(_measured_maps(), idxs)
        @test_throws ArgumentError p12_mark_unscorable!(_measured_maps(), idxs; stack = :bogus)
        ...
        let m = _measured_maps()
            p12_mark_unscorable!(m, [5]; stack = :both)
            m.region_sd[5] = 0.1
            @test_throws ArgumentError _res(m)
        end
    ...
    @testset "constructor guards" begin
        ...
        @test_throws ArgumentError _res(ok; meta = Base.structdiff(m, NamedTuple{(:zscore_arm,)}))
        @test_throws ArgumentError _res(ok; meta = merge(m, (; unexpected = 1)))
    ...
    @testset "the three maps satisfy the structural identity" begin
        ...
        let mm = _measured_maps(s = 0.95, c = -0.95)      # difference = +1.9
            @test _res(mm).region_delta_rho[1] ≈ 1.9
        end
        let mm = _measured_maps(s = -0.95, c = 0.95)      # difference = -1.9
            @test _res(mm).region_delta_rho[1] ≈ -1.9
        end
        ...
        let mm = _measured_maps()
            ...
            rs = _res(mm; meta = _meta(delta_rho_available = false))
            ...
            @test RES_FIX.meta.delta_rho_available === true
        end
    end
    ...
    @testset "the scalar rho is labelled DERIVED (R-1)" begin
        ...
        @test delta_rho(RES_FIX) == RES_FIX.meta.derived_rho
        @test delta_rho(_res(; meta = _meta(derived_rho = -0.11))) == -0.11
    end
    ...
    @testset "a missing operating point reads as NOT CHECKED" begin
        ...
        r = _res(; meta = _meta(ood_threshold = nothing))
        ...
        @test p12_ood_checked(_res(; meta = _meta(ood_threshold = 3.5))) === true
    ...
    @testset "the meta records the z-scoring arm (R-3)" begin
        ...
        @test RES_FIX.meta.zscore_arm === P12_ZSCORE_ARM === :per_row
        @test RES_FIX.meta.schema_version === 1
        @test keys(RES_FIX.meta) === P12_RESULT_META_KEYS
    end
