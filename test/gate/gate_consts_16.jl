...
#########################################################################################
#
# test/gate/gate_consts_16.jl --- the FRESH 16×16 ship-gate pre-registration (D-05, PROD-02).
#
# This is the committed, locked pre-registration for the 16×16 fine grid — instantiated from
# gate_consts_template.jl and committed BEFORE the reported 16×16 gate runs (anti-snooping,
...
#
# 16×16 is the SHIP-WITH-CAVEAT fine grid (summary_dim(16)=512, cont_rows(16)=256). At the coarse
# grids' SBC_IMSIZE=(256,256) each 16×16 patch spans only 16×16=256 px, which is MARGINAL after
# background exclusion — the ≥15-survivor floor is not reliably cleared. So the reported 16×16 gate
# raises SBC_IMSIZE to (512,512): each 32×32 patch then carries 1024 px, comfortably above the
# floor. The reported simulate_pair image size is therefore LARGER here than for the 8×8 and 4×4
# grids by design (07-RESEARCH feasibility: 16×16 is informative only on ≥512² images — the
# minimum-image-size caveat the registry entry must carry). All other SBC/BF/OOD thresholds are the
# same Phase-5-class values the memo §6 pass conditions are stated against, re-expressed FRESH here.
#
# SEED DISCIPLINE (the headline D-05 delta): the reported 16×16 gate rides the FRESH disjoint stream
# prod_rng(16) = Philox4x(PROD_SEED[16], 0), keyed on (PROD_MASTER ⊻ PROD_SALT, 16). PROD_SEED[16]
# is a full-width Philox draw, PROVABLY DISJOINT from — and never reusing —
...
# The grid index G=16 keys a different Philox counter than G=8 and G=4, so PROD_SEED[16] is a
# distinct draw from PROD_SEED[8] and PROD_SEED[4]; it is asserted distinct from the two forbidden
# spike seeds (runtests disjointness test).
#
...
if !isdefined(@__MODULE__, :SBC_M)
    ...
    const SBC_IMSIZE     = (512, 512)  # reported simulate_pair image size — RAISED to 512² for the
                                       # 16×16 grid: each 32×32 patch carries 1024 px, clearing the
                                       # ≥15-survivor floor (256² would be marginal after background
                                       # exclusion). The min-image-size caveat the registry carries.
    ...
    # PROD_SEED[16] is the reproducible, pre-registered fresh 16×16 gate seed. The grid index G=16
    # keys a different Philox counter than G=8 / G=4, so PROD_SEED[16] is a distinct draw.
    ...
    Fresh disjoint gate seeds for the shipped grid family (D-04). `PROD_SEED[16]` is the reported
    16×16 gate seed used to build `prod_rng(16)`; the sibling grids are carried for reference.
    """
